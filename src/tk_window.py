        ...
        if show_folders:
            for dir_name in dirs:
                if search_term.lower() in dir_name.lower():
                    result_listbox.insert(tk.END, f"Folder: {os.path.join(root, dir_name)}")
        
        if show_files:
            for file_name in files:
                if search_term.lower() in file_name.lower() and (not extension_filter or file_name.endswith(extension_filter)):
                    result_listbox.insert(tk.END, f"File: {os.path.join(root, file_name)}")

# Create the main window
root = tk.Tk()
root.title("File Manager")

# Create and configure the search frame
